...
func _ready():
	...
	# Initialization here
	if get_owner() != null:
		get_owner().medicinas_total += value_botiquin
...
func _collect_gear( body ):
	...
	get_owner().get_node("").botiquines += 1
	if get_node("AnimationPlayer").get_current_animation() != "collect":
		if get_owner() != null:
			get_owner().medicinas_collected += value_botiquin
#			get_owner().get_node("gui/Popup_item/medicinas_collected").set_text(str(get_owner().medicinas_collected))
		...
		if get_owner().medicinas_collected == get_owner().medicinas_total:
			get_owner().get_node("gui/splash/label").set_text("Me faltan " + str(resto_botiquines))
