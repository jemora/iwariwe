...
#------------------------------------------ rebote_planta
	...
	plantas = get_node("planta_viva2")
	plantas.set_contact_monitor(true)
	plantas.set_max_contacts_reported(true)
...
#------BALANZAS---------------------------
	...
	if platanos == 18: # colocar uno menos de los que halla que colectar
		platanos = 0
#		misio_fuego()
		...
		grito.set_text("platanos completos")


	if botiquines == 16: # colocar uno menos de los que halla que colectar
		botiquines = 0
#		misio_fuego()
		var mensajes_GUI = get_tree().get_nodes_in_group("mensajes_GUI")
		for reinicio in mensajes_GUI:
		...
		var grito = get_node("gui/splash/label")
		grito.set_text("botiquines completos")
...
#----------- plantas -------------
	...
	var planta = get_node("planta_viva")
	if planta in player.get_colliding_bodies():
		if saltando == 1:
			saltando = 0
			planta.rebotando = 1
			player.rebote_planta = planta.rebote_planta
			var rebote = get_node("Timer_saltando")
			rebote.connect('timeout',self, 'saltando_on')
			rebote.start()

	var planta = get_node("planta_viva1")
	if planta in player.get_colliding_bodies():
		if saltando == 1:
			saltando = 0
			planta.rebotando = 1
	...
	var planta = get_node("planta_viva2")
	if planta in player.get_colliding_bodies():
		if saltando == 1:
			saltando = 0
			planta.rebotando = 1
	...
	var planta = get_node("planta_viva3")
	if planta in player.get_colliding_bodies():
		if saltando == 1:
			saltando = 0
			planta.rebotando = 1
	...
	var planta = get_node("planta_viva4")
	if planta in player.get_colliding_bodies():
		if saltando == 1:
			saltando = 0
			planta.rebotando = 1
	...
	var planta = get_node("planta_viva5")
	if planta in player.get_colliding_bodies():
		if saltando == 1:
			saltando = 0
			planta.rebotando = 1
...
func sonido_cueva_caiman():
	sonido_cueva_caiman.play("ambiente_sapo_grillos")
